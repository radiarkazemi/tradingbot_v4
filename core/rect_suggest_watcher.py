...
chart lines, and NEVER trades. Scans 15M, 5M, AND 1M candles
SIMULTANEOUSLY (all three are independent suggestion sources, not a
confluence/overlap requirement — see
core/rect_suggest_detector.detect_rect_suggestions_mtf) for
consolidation/compression boxes and draws them as suggestions (see
rect_suggest_detector.py for the detection logic and why this is
visualization-only, structurally incapable of placing an order).
"""
import threading
...
from core.rect_suggest_detector import (
    detect_rect_suggestions_mtf, draw_rect_suggestions_on_chart,
    clear_rect_suggestions_on_chart, TIMEFRAME_ORDER,
)
...
class RectSuggestWatcher(threading.Thread):
    ...
    1. Scans 15M+5M+1M candles for consolidation boxes every
       scan_interval seconds, all three as independent sources
    ...
    def __init__(self, symbol: str,
        ...
        self.symbol = symbol
        self.timeframes = timeframes or list(
            TIMEFRAME_ORDER)  # 15M+5M+1M by default
    ...
    def update_settings(self, min_bars: int = None, max_range_atr_mult: float = None,
                        timeframes: list = None, max_draw: int = None,
                        draw_on_chart: bool = None):
        """Hot-update settings without restarting."""
        if min_bars is not None:
            self.min_bars = min_bars
        if max_range_atr_mult is not None:
            self.max_range_atr_mult = max_range_atr_mult
        if timeframes is not None:
            self.timeframes = timeframes
        if max_draw is not None:
            self.max_draw = max_draw
        if draw_on_chart is not None:
            self.draw_on_chart = draw_on_chart
        self._last_count = -1  # force re-log on next scan

    def run(self):
        self._log(
            f"🟧  Rectangle Suggestions started | {self.symbol} "
            f"{'+'.join(self.timeframes)} | min_bars={self.min_bars} "
            f"max_range={self.max_range_atr_mult}×avg-range "
        ...
        while not self._stop.is_set():
            try:
                suggestions = detect_rect_suggestions_mtf(
                    symbol=self.symbol,
                ...
                if len(suggestions) != self._last_count:
                    ...
                    if suggestions:
                        top3 = suggestions[:3]
                        summary = "  ".join(
                            f"🟧{s.timeframe}:{s.height_pips:.1f}p/{s.bars}bars" for s in top3
                        )
                        by_tf = {}
                        for s in suggestions:
                            by_tf[s.timeframe] = by_tf.get(s.timeframe, 0) + 1
                        breakdown = " ".join(
                            f"{tf}={n}" for tf, n in by_tf.items())
                        self._log(
                            f"🟧  Suggestions: {len(suggestions)} box(es) ({breakdown}) | "
                            f"top3: {summary}", "NEW"
                        )
                    else:
                        self._log(
                ...
                if self.draw_on_chart and suggestions:
                    try:
                        draw_rect_suggestions_on_chart(
                            self.symbol, suggestions, max_draw=self.max_draw)
                    except Exception:
                        pass  # EA not running — detection still works

            except Exception as e:
                self._log(
                    f"💥 Rectangle suggestion scan error: {type(e).__name__}: {e}", "ERROR")
