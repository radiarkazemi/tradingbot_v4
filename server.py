...
def start_server(host: str = "0.0.0.0", port: int = 8000):
    ...
    print("  Starting ngrok tunnel (may take 10-30s)...")
    print("  If URL never appears: add authtoken from ngrok.com to:")
    print("  %APPDATA%\\TraderBotV4\\ngrok_token.txt")
    print()

    def on_tunnel_url(url):
        _save_api_key_file()
        if url is None:
            # Auth failure — already logged by tunnel.py
            print()
            print("  ❌  ngrok needs an authtoken (free ngrok account required).")
            print("  Steps:")
            print("    1. Go to https://ngrok.com and sign up (free)")
            print("    2. Copy your authtoken from the dashboard")
            print("    3. Paste it into:")
            print("       %APPDATA%\\TraderBotV4\\ngrok_token.txt")
            print("    4. Restart the bot")
            print()
            return
        print()
        print("  ✅  REMOTE URL (works from anywhere):")
        print(f"      {url}")
        print(f"  API Key: {API_KEY}")
        print("  ============================================================")
        print()
