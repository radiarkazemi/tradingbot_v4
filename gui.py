...
class GUI(QMainWindow):
    ...
    def _build_left(self):
        ...
        self.lot_mode_combo.addItems(["Mode 1 (0.01/touch, max 0.11)",
                                      "Mode 2 (0.02/touch, max 0.20)"])
        self.lot_mode_combo.setCurrentIndex(0 if cfg.SOFT_LOT_MODE == 1 else 1)
        _row("🪜 Soft Lot Mode:", self.lot_mode_combo, cl,
             "Lot at each touch comes from a fixed table instead of "
             "doubling — Mode 1 steps by 0.01/touch, Mode 2 by 0.02/touch "
             "(both cap at the 11th touch, then the kill switch fires)")
    ...
    def _start(self):
        ...
        soft_lot_mode = 1 if self.lot_mode_combo.currentIndex() == 0 else 2
